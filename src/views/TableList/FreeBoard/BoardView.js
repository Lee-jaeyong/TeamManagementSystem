...
export default function BoardView(props) {
  ...
  const deleteYesClick = () => {
    axiosDelete.deleteNotContainsData("http://172.30.1.37:8090/api/teamManage/freeBoard/" + props['data']['data']['seq'],deleteSuccess);
  }
              ...
              if(files['type'] === 'FILE'){
                return (
                  <Chip
                    ...
                    label={files['name']}
                    onClick={() => {
                      axiosGet.getFileDownload("http://172.30.1.37:8090/api/teamManage/freeBoard/" + data['data']['seq'] + "/downloadFile/" + files['name'],files['name']);
                    }}
                    size="small"
                    variant="outlined"
                  />
