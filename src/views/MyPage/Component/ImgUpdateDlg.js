...
export default function ImgUpdateDlg(props) {
  ...
  const updateImage = () => {
    ...
    if(props['originImage']){
      axiosDelete.deleteNotContainsData('http://192.168.43.179:8090/api/users/image',()=>{
        axiosPost.postFileUpload('http://192.168.43.179:8090/api/users/image',(res)=>{successUpdateImage(res)},data);
      });
    }else{
      axiosPost.postFileUpload('http://192.168.43.179:8090/api/users/image',(res)=>{successUpdateImage(res)},data);
    }
