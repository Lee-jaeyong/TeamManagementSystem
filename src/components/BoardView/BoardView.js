...
export default function BoardView(props) {
  ...
  const deleteYesClick = () => {
    axiosDelete.deleteNotContainsData("http://192.168.43.179:8090/api/teamManage/notice/" + props['data']['data']['seq'],deleteSuccess);
  }
              ...
              if(files['type'] === 'FILE'){
                return (
                  <Chip
                    ...
                    label={files['name']}
                    onClick={() => {
                      axiosGet.getFileDownload("http://192.168.43.179:8090/api/teamManage/notice/" + data['data']['seq'] + "/downloadFile/" + files['name'],files['name']);
                    }}
                    size="small"
                    variant="outlined"
                  />
