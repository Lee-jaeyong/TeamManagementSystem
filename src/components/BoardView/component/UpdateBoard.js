...
export default function CreateNotice(props) {
  ...
  const btnSubmit = event => {
    ...
    if (name.current.value.trim() === '') {
      ...
      axiosPut.putContainsData("http://192.168.43.179:8090/api/teamManage/notice/" + props['data']['data']['seq'],createNoticeSuccess,createNoticeError,createNoticeInfo);
    }
  };
  ...
  async function imgUpload(file) {
    ...
    axiosPost.postFileUpload("http://192.168.43.179:8090/api/teamManage/notice/"+props['data']['data']['seq']+"/fileUpload/IMG",successFileUpload,data);  
    setTimeout(() => {
      let originImgs = imgs;
      let checkImgs = [];
  ...
  const fileUpload = file => {
    ...
    for(let i =0;i<file.length;i++)
      data.append("files",file[i]);
    axiosPost.postFileUpload("http://192.168.43.179:8090/api/teamManage/notice/"+props['data']['data']['seq']+"/fileUpload/FILE",successFileUpload,data);    
    setTimeout(() => {
  ...
  const imgHandleDelete = name => {
    axiosPost.postNotContainsData("http://192.168.43.179:8090/api/teamManage/notice/"+props['data']['data']['seq']+"/fileUpload/"+name+"/delete",deleteSuccessImg,deleteErrorImg);
